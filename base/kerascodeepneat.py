...
import logging, random, pydot, copy, uuid, os, sys, csv, json
...
class Individual:
    ...
    def generate(self, save_fig=True, generation=""):
        ...
        for component_id in node_order:
            ...
            #print(type(component.keras_component))
            component.keras_component._name = component.keras_component.name + "_" + uuid.uuid4().hex
            ...
            if component.complementary_component != None:
                ...
                #print(type(component.keras_complementary_component))
                component.keras_complementary_component._name = component.keras_complementary_component.name + "_" + uuid.uuid4().hex
            ...
            #print("component_graph.in_degree(component_id)",component_graph.in_degree(component_id))
        ...
        #self.model = keras.models.Sequential(inputs=model_input, outputs=layer_map[max(layer_map)][-1])
    ...
    def fit(self, input_x, input_y, training_epochs=1, validation_split=0.15, current_generation="", custom_fit_args=None, th_id=0):
        """
        ...
        if custom_fit_args is not None:
            ...
            #else:
            #    fitness = self.model.fit(input_x, input_y, verbose=1, epochs=training_epochs, validation_split=validation_split, batch_size=128)
            fitness = self.model.fit(input_x, input_y, verbose=2, epochs=training_epochs, validation_split=validation_split, batch_size=128)
...
def func2(individuals, custom_fit_args, input_x, input_y, test_x, test_y, training_epochs, validation_split, current_generation, thread_id, iteration, thread_num, intra_size):
    ...
    for index in range(thread_id, len(individuals), thread_num):
        ...
        if (custom_fit_args is not None):
            history = individual.fit(input_x, input_y, training_epochs, validation_split, current_generation=current_generation, custom_fit_args=custom_fit_args, th_id=thread_id)
        else:
            print("self.datasets.custom_fit_args is None")
            history = individual.fit(input_x, input_y, training_epochs, validation_split, current_generation=current_generation, th_id=thread_id)
        score = individual.score(test_x, test_y)
...
class Population:
    ...
    def iterate_fitness(self, training_epochs=1, validation_split=0.15, current_generation=0, thread_num=None, intra_size=1):
        ...
        """
        #conf = tf.ConfigProto(device_count={'CPU': 32}, intra_op_parallelism_threads=4, inter_op_parallelism_threads=4)
        #conf = tf.ConfigProto()
        #conf = tf.compat.v1.ConfigProto(device_count={'CPU': 32})
    ...
    def iterate_generations(self, generations=1, training_epochs=1, validation_split=0.15, mutation_rate=0.5, crossover_rate=0.2, elitism_rate=0.1, possible_components=None, possible_complementary_components=None, thread_num=None, intra_size=1):
        ...
        for generation in range(generations):
            ...
            t = tt
            print("time: ", model_init_time_len)
            #print((f"Generation {generation} spend {model_init_time_len}."))
